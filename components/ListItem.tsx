...
const ListItem: React.FC<ListItemProps> = ({ image, name, href }) => {
    ...
    return (
        <button onClick={onClick} className="relative group flex items-center rounded-md overflow-hidden gap-x-4 bg-neutral-100/10 hover:bg-neutral-100/20 transition pr-4">
            ...
            <div className="absolute transition opacity-0 rounded-full flex items-center justify-center bg-red-400 p-4 drop-shadow-md right-5 group-hover:opacity-100 hover:scale-105">
                <FaPlay />
            </div>
        </button>
    );
